...
pca_contribution_taxa=as.data.frame(get_vars$contrib)
...
ggplot(bc_mds, aes(NMDS1,NMDS2, fill=ibd_Diagnosis)) + geom_point(size=3, pch=21) + theme_bw() + scale_fill_manual(values =c("purple","black","pink2" ,"darkolivegreen3"))
...
#Prepare metabolites for clustering
data_clust=all_new_ID_raw[,colnames(all_new_ID_raw) %in%to_keep$Metabolite]
data_clust=impute_missing_values(data_clust,samples_row = T, method = "knn", missing_filter = 0)
row.names(data_clust)=row.names(all_new_ID_raw)
data_clust2=log10(data_clust)
data_clust2=scale(data_clust2)
...
rgs_mb1=merge(my_regress_phenos,data_clust2, by="row.names")

data_clust3=matrix(nrow=nrow(my_regress_phenos), ncol = ncol(norm_filt))
 n=1
for (i in 7:ncol(rgs_mb1)){

my_lm=lm(rgs_mb1[,i] ~ Amount_sample_gram+run_day_cat+LC.COLUMN+metabolon_Month_in_freezer+ibd_Diagnosis,data = rgs_mb1)
 data_clust3[,n] = my_lm$residuals
 n=n+1
}
data_clust3=as.data.frame(data_clust3)
rownames(data_clust3)=rgs_mb1$Row.names
colnames(data_clust3)=colnames(rgs_mb1)[7:ncol(rgs_mb1)]
...
Cluster samples based on log10 and day-median adjusted metabolitesand clr-transformed bugs 
...
cc_pheno6_t=read.table("~/Desktop/Metabolomics_v2/2.Input/case_control_input_prevalence.txt", header = T, row.names = 1)
cc_pheno6_t=cc_pheno6_t[,1:14]
test_mtb_quantitative2=merge(cc_pheno6_t,q_mtb_t_v3,by="row.names")
...
case_control_quantitative2=test_mtb_quantitative2
case_control_quantitative2$Shannon_Index=NULL
case_control_quantitative2$clinical_Strict_IBS=NULL
case_control_quantitative2$run_day_cat=NULL
case_control_quantitative2$LC.COLUMN=as.factor(case_control_quantitative2$LC.COLUMN)
case_control_quantitative2$Amount_sample_gram=as.numeric(as.character(case_control_quantitative2$Amount_sample_gram))
case_control_quantitative2$clinical_BowelMovementADayDef=as.numeric(as.character(case_control_quantitative2$clinical_BowelMovementADayDef))
case_control_quantitative2$metabolon_Month_in_freezer=as.numeric(as.character(case_control_quantitative2$metabolon_Month_in_freezer))
case_control_quantitative2$host_Age=as.numeric(as.character(case_control_quantitative2$host_Age))
case_control_quantitative2$host_BMI=as.numeric(as.character(case_control_quantitative2$host_BMI))
...
for ( i in 1:12) {
  ...
  if (my_pheno=="clinical_BowelMovementADayDef" | my_pheno=="LC.COLUMN" | my_pheno=="host_BMI"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age") {
    ...
    for (a in 13:ncol(case_control_quantitative2)){
      ...
      my_uni_test[,9]=as.numeric(as.character(my_uni_test[,9]))
      my_samples=nrow(my_uni_test)
      my_lm=summary(lm(my_f,data = my_uni_test ))
      my_lm_coef=as.data.frame(my_lm$coefficients)
      my_lm_pheno=try(my_lm_coef[grep(my_pheno, rownames(my_lm_coef)), ])
...
enzymes=read.table("~/Desktop/Metabolomics_v2/2.Input/EC_with_lldids.txt", header = T, sep="\t", row.names = 1, check.names = F)
enzymes2=merge(cc_rd,t(enzymes), by="row.names")
rownames(enzymes2)=enzymes2$Row.names
enzymes2$id2=NULL
...
oleate=all_new_ID_raw[,"oleate_vaccenate_18_1",drop=F]
PUFAs=cbind(PUFAs, oleate)
#my_flag=1
#for (z in unique(my_batch_id$run_day_cat)){
#  my_names=rownames(my_batch_id)[my_batch_id$run_day_cat==z]
...
# Add phenotypes
cc_ibd=cc_pheno[,"ibd_Diagnosis", drop=F]
...
PUFAs_p$ibd_Diagnosis[PUFAs_p$ibd_Diagnosis=="IBDU"]="2_UC"
PUFAs_p$ibd_Diagnosis[PUFAs_p$ibd_Diagnosis=="CD"]="3_CD"

PUFAs_p2=PUFAs_p[,c("Row.names","ibd_Diagnosis","arachidonate_20_4n6","dihomolinoleate_20_2n6","docosadienoate_22_2n6","hexadecadienoate_16_2n6","linoleate_18_2n6","docosahexaenoate_DHA__22_6n3","docosapentaenoate_DPA__22_5n3","eicosapentaenoate_EPA__20_5n3","hexadecatrienoate_16_3n3","stearidonate_18_4n3","oleate_vaccenate_18_1" ,"n6_vs_n3")]
...
PUFAs_p2$oleate_vaccenate_18_1=log10(PUFAs_p2$oleate_vaccenate_18_1)

PUFAs_p2.1=melt(PUFAs_p2,id.var=c("Row.names","ibd_Diagnosis"))

my_comparisions=list(  c("1_Control","2_UC"), c("3_CD","2_UC"),c("3_CD","1_Control"))

ggplot(PUFAs_p2.1, aes(ibd_Diagnosis,value, fill=ibd_Diagnosis)) + geom_violin(alpha=0.8) + geom_jitter(height = 0, width = 0.1, alpha=0.4) + geom_boxplot(width=0.1, fill="white") + theme_bw() + xlab("Disease")  + scale_fill_manual(values =c("gray80","steelblue2","firebrick3" )) + facet_wrap(variable~., ncol=6) + stat_compare_means(comparisons = my_comparisions, method = "wilcox.test", p.adjust.methods="bonferroni",label="p.signif") + ylab ("log10 transformed AUC values") 
...
pufas_pheno=ibd_test_phenos[, c("clinical_ActiveDisease","clinical_Diagnosis")]
pufas_pheno$clinical_Diagnosis=as.character(pufas_pheno$clinical_Diagnosis)
pufas_pheno$clinical_Diagnosis[pufas_pheno$clinical_Diagnosis=="IBDU"]="UC"
...
ggplot(PUFAs_p3.1, aes(clinical_Diagnosis,value, fill=clinical_Diagnosis)) + geom_violin(alpha=0.8) + geom_jitter(height = 0, width = 0.1, alpha=0.4) + geom_boxplot(width=0.1, fill="white") + theme_bw() + xlab("Clusters")  + scale_fill_manual(values =c("gray80","lightblue","steelblue2","salmon","firebrick3" )) + facet_wrap(variable~., ncol=6) + stat_compare_means(comparisons = my_comparisions, method = "wilcox.test", p.adjust.methods="bonferroni",label="p.signif", label.y=c(11,11,11.2,12)) +theme(axis.title.x=element_blank(),axis.text.x=element_blank(), axis.ticks.x=element_blank())
...
pufas_pheno=ibd_test_phenos[, c("clinical_Location","clinical_Diagnosis")]
pufas_pheno$clinical_Diagnosis=as.character(pufas_pheno$clinical_Diagnosis)
pufas_pheno$clinical_Diagnosis[pufas_pheno$clinical_Diagnosis=="IBDU"]="UC"
...
Metabolites associated to IBD => Remove patients with any resection
---
...
#################
### Remove any resection
#################

iresec=ibd_test_phenos[,c("clinical_ResectionAny"), drop=F]
case_control_quantitative2=merge(iresec,case_control_quantitative,all.y=T, by="row.names")
case_control_quantitative2$clinical_ResectionAny[is.na(case_control_quantitative2$clinical_ResectionAny)]="no"
case_control_quantitative2=subset(case_control_quantitative2,case_control_quantitative2$clinical_ResectionAny=="no")
rownames(case_control_quantitative2)=case_control_quantitative2$Row.names
case_control_quantitative2$Row.names=NULL
case_control_quantitative2$clinical_ResectionAny=NULL
...
associations_ibd_resec$FDR=p.adjust(associations_ibd_resec$`Pr(>|t|)`,method = "BH")
associations_ibd_resec$Bonferroni=p.adjust(associations_ibd_resec$`Pr(>|t|)`,method = "bonferroni")
write.table(associations_ibd_resec, "~/Desktop/Metabolomics_v2/3.Preliminary_results/IBD_associations_quantitative_removed_any_resection.txt", sep = "\t", quote = F)

```



Test differences in taxa (for comparison)

XX. Metabolites associated to IBD
---
...
for ( i in 1:11) {
  ...
  if (my_pheno=="clinical_BowelMovementADayDef" | my_pheno=="PF_RD" | my_pheno=="host_BMI" | my_pheno=="host_Sex" | my_pheno=="host_Age") {
    ...
    for (a in 12:ncol(case_control_bacteria)){
      ...
      my_uni_test=case_control_bacteria[,c("host_Sex","host_Age","clinical_BowelMovementADayDef","host_BMI","host_BMI",my_pheno,my_trait)]
      my_preds=c("host_Sex","host_Age","clinical_BowelMovementADayDef", "host_BMI","host_BMI",my_pheno)
      my_f=as.formula(paste(my_trait, paste(my_preds, collapse = " + "), sep = " ~ "))
      #my_uni_test[,8]=as.numeric(as.character(my_uni_test[,8]))
      my_samples=nrow(my_uni_test)
      my_lm=summary(lm(my_f,data = my_uni_test ))
...
write.table(associations_ibd_bugs, "~/Desktop/Metabolomics_v2/3.Preliminary_results/IBD_associations_BUGS.txt", sep = "\t", quote = F)
