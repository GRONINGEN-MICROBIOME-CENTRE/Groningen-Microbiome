...
clus_sel$Clus[clus_sel$K.4==2]="D"
...
for (a in 3:ncol(cb)){
  ...
  yy= pairwise.wilcox.test(cb[,a],cb$Clus, p.adjust.method = "none")$p.value
...
write.table(a_report_phe,"~/Desktop/Metabolomics_for_cluster/3.Preliminary_results_v2/iCluster_enriched_pheno.txt", quote = F, sep = "\t")
...
ggplot(ct, aes(as.factor(Clus),tryptophan, fill=as.factor(Clus))) + geom_violin(alpha=0.8) + geom_jitter(height = 0, width = 0.1, alpha=0.4) + geom_boxplot(width=0.1, fill="white") + theme_bw() + xlab("Clusters")  + scale_fill_manual(values =c("gray80","steelblue2","gold3","firebrick3" ))
...
ggplot(cm2, aes(as.factor(Clus),cytidine, fill=as.factor(Clus))) + geom_violin(alpha=0.8) + geom_jitter(aes(shape=ibd_Diagnosis),height = 0, width = 0.1, alpha=0.4) + geom_boxplot(width=0.1, fill="white") + theme_bw() + xlab("Clusters")  + scale_fill_manual(values =c("gray80","steelblue2","gold3","firebrick3" ))
...
for (z in unique(my_batch_id$run_day_cat)){
  my_names=rownames(my_batch_id)[my_batch_id$run_day_cat==z]
  d1=subset(all_new_ID_raw, row.names(all_new_ID_raw) %in% my_names)
  d1.1=apply(d1, 2, function (x) {x/median(x,na.rm = T)})
...
BA_sel=BA[,colnames(BA) %in% PBA | colnames(BA) %in% SBA]
BA_sel$Primary_BA=rowSums(BA_sel[,colnames(BA_sel) %in% PBA], na.rm = T)
BA_sel$Secondary_BA=rowSums(BA_sel[,colnames(BA_sel) %in% SBA], na.rm = T)
BA_sel$Ratio_Secon_Primary=BA_sel$Secondary_BA/BA_sel$Primary_BA
BA_sel$Ratio_log=log10(BA_sel$Ratio_Secon_Primary)
PSC=phenos_ibd[ ,"clinical_PSC", drop=F]
BA_sel_pheno=merge(cc_ibd,BA_sel,by="row.names")
rownames(BA_sel_pheno)=BA_sel_pheno$Row.names
BA_sel_pheno$Row.names=NULL
BA_sel_pheno=merge(BA_sel_pheno,PSC,by="row.names", all.x = T)
BA_sel_pheno$clinical_PSC[is.na(BA_sel_pheno$clinical_PSC)]="no"
BA_sel_pheno$ibd_Diagnosis2=as.character(BA_sel_pheno$ibd_Diagnosis)
BA_sel_pheno$ibd_Diagnosis2[BA_sel_pheno$clinical_PSC=="yes"]="PSC"
rownames(BA_sel_pheno)=BA_sel_pheno$Row.names
BA_sel_pheno$Row.names=NULL

#BA_sel_pheno$ibd_Diagnosis=as.character(BA_sel_pheno$ibd_Diagnosis)
BA_sel_pheno$ibd_Diagnosis2[BA_sel_pheno$ibd_Diagnosis2=="Control"]="A_control"

compare=list( c("CD","A_control"),c("CD","PSC"),  c("UC","PSC"),   c("CD","UC"),c("A_control","PSC"),c("A_control","UC"))
#BA_sel_plot=BA_sel_pheno[,c("ibd_Diagnosis","Ratio_log")]
#BA_sel_plot=melt(BA_sel_plot)
ggplot (BA_sel_pheno, aes (ibd_Diagnosis2,Ratio_log, fill=ibd_Diagnosis2)) + geom_violin() + geom_boxplot(width=0.1,position=position_dodge(1), alpha=0.2)  + theme_bw() +scale_fill_manual(values=c("snow","firebrick2","grey80","yellow","blue2"))  + stat_compare_means(comparisons = compare, method = "wilcox.test") + ylab ("log10( Secondary/Primary Bile Acids)") + xlab ("Cohort") + scale_x_discrete(labels=c("n=255","n=233","n=11","n=21", "n=159"))

## Remove BA meds users ##

ibd_ba_meds=rownames(phenos_ibd)[phenos_ibd$med_BileAcids=="User"]
BA_sel_pheno2=subset(BA_sel_pheno,BA_sel_pheno$Row.names%ni%ibd_ba_meds)

## Test typical primary and secondary ratios ## 

BA_sel_pheno2$Classical_PBA=rowSums(BA_sel_pheno2[,c("cholate","chenodeoxycholate")], na.rm=T)
BA_sel_pheno2$Classical_SBA=rowSums(BA_sel_pheno2[,c("deoxycholate", "lithocholate")], na.rm=T)
BA_sel_pheno2$CA_and_CDCA_vs_DCA_and_LCA=log10(BA_sel_pheno2$Classical_SBA/BA_sel_pheno2$Classical_PBA)
BA_sel_pheno2$ibd_Diagnosis=as.character(BA_sel_pheno2$ibd_Diagnosis)
BA_sel_pheno2$ibd_Diagnosis[BA_sel_pheno2$ibd_Diagnosis=="Control"]="A_control"

compare=list( c("CD","A_control"), c("CD","UC"),c("A_control","UC"))
BA_sel_pheno2.1=subset(BA_sel_pheno2,BA_sel_pheno2$ibd_Diagnosis!="IBDU")

ggplot (BA_sel_pheno2, aes (ibd_Diagnosis,CA_and_CDCA_vs_DCA_and_LCA, fill=ibd_Diagnosis)) + geom_violin() + geom_boxplot(width=0.1,position=position_dodge(1), alpha=0.2)  + theme_bw() +scale_fill_manual(values=c("snow","firebrick2","grey80","blue2"))  + stat_compare_means(comparisons = compare, method = "wilcox.test") + ylab ("log10( Secondary/Primary Bile Acids)") + xlab ("Cohort") + scale_x_discrete(labels=c("n=255","n=233","n=10", "n=158"))



## Test typical primary conjugated and unconjugated ratios ## 


BA_sel_pheno2$PBA_conjugated=rowSums(BA_sel_pheno2[,c("glycocholate", "taurocholate", "glycochenodeoxycholate", "taurochenodeoxycholate")], na.rm=T)
BA_sel_pheno2$PBA_con_vs_uncon=log10(BA_sel_pheno2$PBA_conjugated/BA_sel_pheno2$Classical_PBA)

BA_sel_pheno2$CA_conjugated=rowSums(BA_sel_pheno2[,c("glycocholate", "taurocholate")], na.rm=T)
BA_sel_pheno2$CA_con_vs_uncon=log10(BA_sel_pheno2$CA_conjugated/BA_sel_pheno2$cholate)


CD_resec=phenos_ibd[,c("ibd_ResectionIlealAny", "ibd_ResectionIlealCecalAny","ibd_DiseaseLocation", "ibd_ActiveDisease")]
CD_resec$SI_surgery="C_intact_SI"
CD_resec$SI_surgery[CD_resec$ibd_ResectionIlealAny=="yes" | CD_resec$ibd_ResectionIlealCecalAny=="yes"]="D_resect_SI"
CD_resec=CD_resec[,"SI_surgery", drop=F]
rownames(BA_sel_pheno2)=BA_sel_pheno2$Row.names
BA_sel_pheno2$Row.names=NULL
BA_sel_pheno2=merge(CD_resec,BA_sel_pheno2,by="row.names", all.y = T)
BA_sel_pheno2$SI_surgery[BA_sel_pheno2$ibd_Diagnosis=="UC"]="B_UC"
BA_sel_pheno2$SI_surgery[BA_sel_pheno2$ibd_Diagnosis=="A_control"]="A_control"

#compare=list( c("A_control","B_UC"), c("B_UC","C_intact_SI"),c("C_intact_SI","D_resect_SI"), c("A_control","C_intact_SI"),c("B_UC","D_resect_SI"),c("A_control","D_resect_SI"))

compare=list( c("A_control","B_UC"), c("B_UC","C_intact_SI"),c("C_intact_SI","D_resect_SI"),c("A_control","D_resect_SI"))

#ggplot (BA_sel_pheno2, aes (SI_surgery,CA_and_CDCA_vs_DCA_and_LCA, fill=SI_surgery)) + geom_violin() + geom_boxplot(width=0.1,position=position_dodge(1), alpha=0.2)  + theme_bw() +scale_fill_manual(values=c("snow","blue2","salmon2", "firebrick2"))  + stat_compare_means(comparisons = compare, method = "wilcox.test") + ylab ("log10( Secondary/Primary Bile Acids)") + xlab ("Cohort") + scale_x_discrete(labels=c("n=255","n=158","n=156", "n=87"))
BA_sel_pheno3=BA_sel_pheno2[!is.infinite(BA_sel_pheno2$CA_and_CDCA_vs_DCA_and_LCA),]

ggplot (BA_sel_pheno3, aes (SI_surgery,log10(cholate), fill=SI_surgery)) + geom_violin(alpha=0.8) + geom_jitter(height = 0, width = 0.1, alpha=0.4) + geom_boxplot(width=0.1,position=position_dodge(1), alpha=0.2)  + theme_bw() +scale_fill_manual(values=c("snow","blue2","salmon2", "firebrick2"))  + stat_compare_means(comparisons = compare, method = "wilcox.test") + ylab ("log10( Cholic acid)") + xlab ("Cohort") + scale_x_discrete(labels=c("n=255","n=158","n=156", "n=87"))
...
my_regress_phenos$host_BMI[is.na(my_regress_phenos$host_BMI)]=median(my_regress_phenos$host_BMI,na.rm = T)
...
################################
#                             #
#   Correlation metabolites   #
#                             #
###############################

mtb_matrix=cor(rgs_mb2, method = "spearman")
mtb_matrix_dis=as.dist(1- mtb_matrix)
mtb_matrix_tree=hclust(mtb_matrix_dis, method = "ward.D")

mtb_matrix=rcorr(as.matrix(rgs_mb2), type = "spearman")
mtb_matrix=corr.test(as.matrix(rgs_mb2), method  = "spearman", adjust="BH")

#Function: http://www.sthda.com/english/wiki/correlation-matrix-a-quick-start-guide-to-analyze-format-and-visualize-a-correlation-matrix-using-r-software
...
mtb_matrix_corr=flattenCorrMatrix(mtb_matrix$r, mtb_matrix$P)

write.table(mtb_matrix_corr, "~/Desktop/Metabolomics_v2/3.Preliminary_results/1.Summary_stats/Correlation_metabolites_regressed.txt")
...
##test correlation between bacterai and metabolites##

##IBD regressed values##

corr_mtb_taxa=merge(rgs_mb3,rgs_tx3, by="row.names")
...
my_IBD=rownames(cc_ibd)[cc_ibd$ibd_Diagnosis!="Control"]
my_CNT=rownames(cc_ibd)[cc_ibd$ibd_Diagnosis=="Control"]
...
write.table(corr_taxa_metabolites_f, "~/Desktop/Metabolomics_v2/3.Preliminary_results/1.Summary_stats/Correlation_metabolites_taxa_spearman.txt", sep = "\t", quote = F)
...
summary_bgc=summary_stats(bgc_rpkm3,select2, missing_vals = 0)
...
17.Enzime commission abundance
===
...
cnt_test_bgc=read.table("./controls_EC.txt", sep = "\t", header = T, row.names = 1)
...
for ( i in 1:1125) {
  ...
  if (my_pheno=="clinical_BowelMovementADayDef" | my_pheno=="LC.COLUMN" | my_pheno=="host_BMI"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 1126:ncol(cnt_test_bgc)){
      my_trait=colnames(cnt_test_bgc)[a]
      my_uni_test=cnt_test_bgc[,c("host_Sex","host_Age","clinical_BowelMovementADayDef","host_BMI","LC.COLUMN","Amount_sample_gram","metabolon_Month_in_freezer",my_pheno,my_trait)]
      my_preds=c("host_Sex","host_Age","clinical_BowelMovementADayDef", "host_BMI","LC.COLUMN","Amount_sample_gram","metabolon_Month_in_freezer",my_pheno)
      my_f=as.formula(paste(my_trait, paste(my_preds, collapse = " + "), sep = " ~ "))
...
18.Bacterial pathways (MetaCyc) abundance
===

```{r}

ptw=read.table("~/Desktop/Metabolomics_for_cluster/2.Input/Merged_metacyc_uniref90_filtered.tsv", header = T, sep="\t", row.names = 1, check.names = F)

colnames(ptw)=gsub("_pathabundance","",colnames(ptw))

ptw2=merge(cc_rd,t(ptw), by="row.names")
rownames(ptw2)=ptw2$id2
ptw2$id2=NULL
ptw2$Row.names=NULL
ptw2$IDs_MGS=NULL
ptw2$PF_RD=NULL
ptw3=subset(ptw2, row.names(ptw2) %in% row.names(cc_pheno2))
#summary_enzymes=summary_stats(enzymes3,select2, missing_vals = 0)

features_per_samples=data.frame(Sample=rownames(ptw3), Counts=rowSums(ptw3!=0))
samples_per_feature=data.frame(BCG=colnames(ptw3), Counts=colSums(ptw3!=0))

ggplot(features_per_samples, aes(reorder(Sample,-Counts), Counts )) + geom_bar(stat = "identity", fill="lightblue3") + theme_classic() + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + ylab ("Number of MetaCyc pathways per samples")  + xlab ("Samples")

boxss=merge(cc_pheno, features_per_samples, by="row.names")

ggplot(boxss, aes (ibd_Diagnosis, Counts, fill=ibd_Diagnosis)) + geom_violin() + geom_boxplot(width=0.1,position=position_dodge(1), alpha=0.2)  + theme_bw() +scale_fill_manual(values=c("firebrick2","white","gray80", "blue2")) + ylab ("Number of pathways detected") + xlab ("") + scale_x_discrete(labels=c("n=239","n=255", "n=12", "n=175"))

ggplot(samples_per_feature, aes(reorder(BCG,-Counts), Counts )) + geom_bar(stat = "identity", fill="gray50") + theme_classic() + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + ylab ("Number of samples")  + xlab ("Pathways")


ptw3$UNMAPPED=NULL
ptw3$UNINTEGRATED=NULL

#enzymes4=enzymes3[,((colSums(enzymes3 !=0) / nrow(enzymes3)) *100 )>20]


#Filtering prevalence in 20% 
ptw_trans=transform_and_filter_taxa(ptw3, samples_row = T, method = "clr",missing_filter = 20)
ptw_corr=cor(ptw_trans, method = "spearman")
corr_ptw <- findCorrelation(ptw_corr, cutoff=0.85, names = T)
ptw_trans2=ptw_trans[,colnames(ptw_trans) %ni% corr_ptw]

write.table(ptw_trans, "~/Desktop/Metabolomics_for_cluster/2.Input/Pathways_filtered.txt", sep = "\t", quote = F)

ptw_ibd=merge(cc_ibd,ptw_trans, by="row.names")

flag=1
for (a in 3:ncol(ptw_ibd)){
  my_name=colnames(ptw_ibd)[a]
  yy= pairwise.wilcox.test(ptw_ibd[,a],ptw_ibd$ibd_Diagnosis, p.adjust.method = "none")$p.value
  ccb=data.frame("Feature"=my_name,"CD_Control"=yy[1,1], "CD_IBDU"=yy[2,1],"CD_UC"=yy[3,1],"Control_IBDU"=yy[2,2], "Control_UC"=yy[3,2],"IBDU_UC"=yy[3,3] )
  if(flag==1){
    ptw_res=ccb
    flag=5
  }else{
    ptw_res=rbind(ptw_res,ccb)
  }
}
ptw_res$CD_IBDU=NULL
ptw_res$Control_IBDU=NULL
ptw_res$IBDU_UC=NULL
ptw_res_2=melt(ptw_res)
ptw_res_2$FDR=p.adjust(ptw_res_2$value, method = "BH")
ptw_res_2=dcast(ptw_res_2, formula=Feature~variable, value.var="FDR")


#a_report_bgc=merge(summary_bgc,bgc_res_2, by.x="Metabolite", by.y="Feature")

#######################################
## correlation pathways & metabolites ##
#######################################


rgs_ptw=merge(my_regress_phenos,ptw_trans, by="row.names")

rgs_ptw3=matrix(nrow=nrow(my_regress_phenos), ncol = ncol(ptw_trans))
n=1
for (i in 12:ncol(rgs_ptw)){

  #my_lm=lm(rgs_tx[,i] ~ host_Age_sampling+ host_BMI+ host_Sex +PF_RD+clinical_BowelMovementADayDef+ ibd_Diagnosis,data = rgs_tx)
  
  #rgs_tx2[,n] = my_lm$residuals
  
  my_lm2=lm(rgs_ptw[,i] ~host_Age_sampling+ host_BMI+ host_Sex+clinical_BowelMovementADayDef+ PF_RD,data = rgs_ptw)
  
  rgs_ptw3[,n] = my_lm2$residuals
  n=n+1
}
rgs_ptw3=as.data.frame(rgs_ptw3)
rownames(rgs_ptw3)=rgs_ptw$Row.names
colnames(rgs_ptw3)=colnames(rgs_ptw)[12:ncol(rgs_ptw)]



ptw_mtb=merge(rgs_ptw3,rgs_mb3, by="row.names")
rownames(ptw_mtb)=ptw_mtb$Row.names
ptw_mtb$Row.names=NULL

ptw_mtb_IBD=subset(ptw_mtb,row.names(ptw_mtb) %in% my_IBD)
ptw_mtb_CNT=subset(ptw_mtb,row.names(ptw_mtb) %in% my_CNT)


ptw_mtb_IBD_val=rcorr(as.matrix(ptw_mtb_IBD), type = "spearman")

test_corr_coef=ptw_mtb_IBD_val$r
test_corr_coef_sel=test_corr_coef[(ncol(rgs_ptw3)+1):nrow(test_corr_coef), 1:ncol(rgs_ptw3)]
test_corr_pval=ptw_mtb_IBD_val$P
test_corr_pval_sel=test_corr_pval[(ncol(rgs_ptw3)+1):nrow(test_corr_pval), 1:ncol(rgs_ptw3)]
corr_coef=melt(test_corr_coef_sel)
corr_pval=melt(test_corr_pval_sel)
...
colnames(corr_coef)=c("Metabolite","Pathway","Coeff")
colnames(corr_pval)=c("Metabolite","Pathway","P_val")
corr_mtb_pwy_oIBD=cbind(corr_coef, corr_pval)
corr_mtb_pwy_oIBD=data.frame(corr_mtb_pwy_oIBD)

ptw_mtb_vals=rcorr(as.matrix(ptw_mtb_CNT), type = "spearman")
#here:
test_corr_coef=ptw_mtb_vals$r
test_corr_coef_sel=test_corr_coef[(ncol(rgs_ptw3)+1):nrow(test_corr_coef), 1:ncol(rgs_ptw3)]
test_corr_pval=ptw_mtb_vals$P
test_corr_pval_sel=test_corr_pval[(ncol(rgs_ptw3)+1):nrow(test_corr_pval), 1:ncol(rgs_ptw3)]
corr_coef=melt(test_corr_coef_sel)
corr_pval=melt(test_corr_pval_sel)
...
colnames(corr_coef)=c("Metabolites","Pathway","Coeff")
colnames(corr_pval)=c("Metabolites","Pathway","P_val")
corr_ptw_mtb_CNT=cbind(corr_coef, corr_pval)

corr_mtb_pwy_oIBD$Bonferroni=p.adjust(corr_mtb_pwy_oIBD$P_val, method = "bonferroni")
corr_ptw_mtb_CNT$Bonferroni=p.adjust(corr_ptw_mtb_CNT$P_val, method = "bonferroni")
corr_mtb_pwy_oIBD$FDR=p.adjust(corr_mtb_pwy_oIBD$P_val, method = "BH")
corr_ptw_mtb_CNT$FDR=p.adjust(corr_ptw_mtb_CNT$P_val, method = "BH")

corr_mtb_ptw_f=cbind(corr_mtb_pwy_oIBD, corr_ptw_mtb_CNT)


colnames(corr_mtb_ptw_f)=c("Metabolites", "Pathway", "Coef_IBD", "Metabolites","Enzyme","P-val_IBD","BFN_IBD","FDR_IBD", "Metabolites", "Pathway","Coeff_Controls","Metabolites","Pathway", "P-val_Controls", "BFN_Controls", "FDR_Controls")

corr_mtb_ptw_f$replicated="No"
corr_mtb_ptw_f$replicated[corr_mtb_ptw_f$FDR_IBD<0.05 & corr_mtb_ptw_f$FDR_Controls<0.05 ]="Yes"


write.table(corr_mtb_ptw_f, "~/Desktop/Metabolomics_for_cluster/3.Preliminary_results_v2/3.Correlations_layers/correlation_pathway_metabolites_spearman.txt", sep = "\t", quote = F)
