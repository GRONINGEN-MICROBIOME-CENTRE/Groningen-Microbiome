...
cc_pheno4$Row.names=NULL
cc_pheno5=merge(my_batch_id,cc_pheno4, by="row.names")
cc_rd=read.table("~/Documents/Project_Metabolome/1.Input_files/IDs_read_depth.txt", header = T, row.names = "V5")
cc_rd$row.names=NULL
cc_rd2=merge(cc_rd,cc_pheno5,by="row.names")
...
for (i in 12:ncol(rgs_mb)){
  ...
  my_lm=lm(rgs_mb[,i] ~ run_day_cat+Amount_sample_gram+LC.COLUMN+metabolon_Month_in_freezer+host_Age_sampling+ host_BMI+ host_Sex +clinical_BowelMovementADayDef+ ibd_Diagnosis,data = rgs_mb)
  
  rgs_mb2[,n] = my_lm$residuals
  
  my_lm2=lm(rgs_mb[,i] ~ run_day_cat+Amount_sample_gram+LC.COLUMN+metabolon_Month_in_freezer+host_Age_sampling+ host_BMI+ host_Sex+ clinical_BowelMovementADayDef,data = rgs_mb)
...
for (i in 12:ncol(rgs_tx)){
  ...
  my_lm=lm(rgs_tx[,i] ~ host_Age_sampling+ host_BMI+ host_Sex +PF_RD+clinical_BowelMovementADayDef+ ibd_Diagnosis,data = rgs_mb)
  
  rgs_tx2[,n] = my_lm$residuals
  
  my_lm2=lm(rgs_tx[,i] ~host_Age_sampling+ host_BMI+ host_Sex+clinical_BowelMovementADayDef+ PF_RD,data = rgs_mb)
...
colnames(corr_taxa_metabolites_f)=c("Taxa", "Metabolite", "Coef_IBD", "P-val_IBD","BFN_IBD", "Taxa", "Metabolites","Coeff_Controls", "P-val_Controls", "BFN_Controls")
...
#ml conda activate /home/umcg-avich/miniconda3/envs/BiG-MAP_process
#ml Bowtie2
...
bgc2=bgc[,colnames(bgc) %in% row.names(cc_pheno2)]

features_per_samples=data.frame(Sample=colnames(bgc2), Counts=colSums(bgc2!=0))
samples_per_feature=data.frame(BCG=rownames(bgc2), Counts=rowSums(bgc2!=0))
...
bgc_filter=bgc2[, colnames(bgc2) %ni%  remove_samples]
...
n=1
for (i in 12:ncol(bgc_rgs)){

  my_lm=lm(bgc_rgs[,i] ~ run_day_cat+Amount_sample_gram+LC.COLUMN+metabolon_Month_in_freezer+host_Age_sampling+ host_BMI+ host_Sex+ clinical_BowelMovementADayDef,data = bgc_rgs)
...
colnames(bgc_rgs2)=colnames(bgc_rgs)[12:ncol(bgc_rgs)]


rgs_mb4=rgs_mb3[row.names(rgs_mb3) %in% row.names(bgc_rgs2),]
...
corr_mtb_bgc_IBD=subset(test_corr,row.names(test_corr) %in% my_IBD)
corr_mtb_bgc_CNT=subset(test_corr,row.names(test_corr) %in% my_CNT)


corr_mtb_bgc_vals=rcorr(as.matrix(corr_mtb_bgc_IBD), type = "spearman")

test_corr_coef=corr_mtb_bgc_vals$r
test_corr_coef_sel=test_corr_coef[(ncol(rgs_mb4)+1):nrow(test_corr_coef), 1:ncol(rgs_mb4)]
test_corr_pval=corr_mtb_bgc_vals$P
...
colnames(corr_coef)=c("Taxa","Metabolites","Coeff")
colnames(corr_pval)=c("Taxa","Metabolites2","P_val")
corr_bgc_metabolites_oIBD=cbind(corr_coef, corr_pval)


corr_mtb_bgc_vals=rcorr(as.matrix(corr_mtb_bgc_CNT), type = "spearman")
...
colnames(corr_coef)=c("Taxa","Metabolites","Coeff")
colnames(corr_pval)=c("Taxa","Metabolites2","P_val")
corr_bgc_metabolites_CNT=cbind(corr_coef, corr_pval)
