...
write.table(q_mtb, "~/Documents/Project_Metabolome/6.Results/knn_Imputed_metabolites_clr.txt", sep = "\t",quote = F)
...
cor_pval=melt(cor_metabolites_pval)
cor_coef$pvalue=cor_pval$value
write.table(cor_coef, "~/Documents/Project_Metabolome/6.Results/072020/Correlation_metabolites_IBD.txt", sep = "\t", quote = F)
...
cor_pval=melt(cor_metabolites_pval)
cor_coef$pvalue=cor_pval$value
write.table(cor_coef, "~/Documents/Project_Metabolome/6.Results/072020/Correlation_metabolites_CNT.txt", sep = "\t", quote = F)
...
#write.table(clus_mtb, "~/Documents/Project_Metabolome/1.Input_files/clst_mtb2", sep="\t", quote = F)
#write.table(clus_pheno, "~/Documents/Project_Metabolome/1.Input_files/clst_pheno2", sep="\t", quote = F)
#write.table(clus_taxa, "~/Documents/Project_Metabolome/1.Input_files/clst_taxa2", sep="\t", quote = F)
...
clus_mtb=read.table("~/Documents/Project_Metabolome/1.Input_files/clst_mtb2", sep = "\t", header = T, row.names = 1)
clus_taxa=read.table("~/Documents/Project_Metabolome/1.Input_files/clst_taxa2", sep = "\t", header = T, row.names = 1)
clus_pheno=read.table("~/Documents/Project_Metabolome/1.Input_files/clst_pheno2", sep = "\t", header = T, row.names = 1)
...
setwd("~/Documents/Project_Metabolome/6.Results/iClust/new/")
output=alist() 
...
write.table(clusters, file="~/Documents/Project_Metabolome/6.Results/072020/clusterMembership_iCluster.txt",sep='\t',quote=F)
...
#capture.output(sigfeatures, file= "~/Documents/Project_Metabolome/6.Results/072020/features_icluster.txt")
...
clus_sel$`K=4`=as.factor(clus_sel$`K=4`)
...
for (a in 3:ncol(cm)){
  ...
  yy= pairwise.wilcox.test(cm[,a],cm$`K=4`, p.adjust.method = "none")$p.value
  cc=data.frame("Feature"=my_name,"C1_C2"=yy[1,1], "C1_C3"=yy[2,1],"C1_C4"=yy[3,1],"C2_C3"=yy[2,2], "C2_C4"=yy[3,2],"C3_C4"=yy[3,3] )
  if(flag==1){
    clus_res_mtb=cc
    flag=5
  }else{
...
#write.table(a_report_mtb, "~/Documents/Project_Metabolome/6.Results/072020/iCluster_enriched_metabolites.txt", quote = F, sep = "\t")
...
for (a in 3:ncol(ct)){
  ...
  cct=data.frame("Feature"=my_name,"C1_C2"=yy[1,1], "C1_C3"=yy[2,1],"C1_C4"=yy[3,1],"C2_C3"=yy[2,2], "C2_C4"=yy[3,2],"C3_C4"=yy[3,3] )
  if(flag==1){
    clus_res_taxa=cct
    flag=5
  }else{
...
#write.table(a_report_taxa, "~/Documents/Project_Metabolome/6.Results/072020/iCluster_enriched_taxa.txt", quote = F, sep = "\t")
...
for (a in 3:ncol(cp)){
  ...
  if (is.numeric(cp[,a])){
    yy= pairwise.wilcox.test(cp[,a],cm$`K=4`, p.adjust.method = "none")$p.value
    ccb=data.frame("Feature"=my_name,"C1_C2"=yy[1,1], "C1_C3"=yy[2,1],"C1_C4"=yy[3,1],"C2_C3"=yy[2,2], "C2_C4"=yy[3,2],"C3_C4"=yy[3,3] )
    if(flag==1){
      clus_res_phe=ccb
      flag=5
    ...
    tmpdf=cp[,c(2,a)]
    zzz=glm(tmpdf[,2] ~ Clusters, family = binomial(), data = tmpdf)
    yyy=as.data.frame(pairs(regrid(lsmeans(zzz,"Clusters"), p.adjust.methods="none")))
    ccb=data.frame("Feature"=my_name,"C1_C2"=yyy[1,6], "C1_C3"=yyy[2,6],"C1_C4"=yyy[3,6],"C2_C3"=yyy[4,6], "C2_C4"=yyy[5,6],"C3_C4"=yyy[6,6] )
    if(flag==1){
      clus_res_phe=ccb
      flag=5
    }else{
...
stats_phenos_clus=describe.by(cp, cp$Clusters)
stats_phenos=data.frame("Phenotype"=colnames(cp), C1_N= stats_phenos_clus$`1`$n, C1_Min=stats_phenos_clus$`1`$min, C1_Max=stats_phenos_clus$`1`$max, C1_Mean=stats_phenos_clus$`1`$mean, C1_Median=stats_phenos_clus$`1`$median, C1_SD=stats_phenos_clus$`1`$sd,C2_N= stats_phenos_clus$`2`$n, C2_Min=stats_phenos_clus$`2`$min, C2_Max=stats_phenos_clus$`2`$max, C2_Mean=stats_phenos_clus$`2`$mean, C2_Median=stats_phenos_clus$`2`$median, C2_SD=stats_phenos_clus$`2`$sd , C3_N= stats_phenos_clus$`3`$n, C3_Min=stats_phenos_clus$`3`$min, C3_Max=stats_phenos_clus$`3`$max, C3_Mean=stats_phenos_clus$`3`$mean, C3_Median=stats_phenos_clus$`3`$median, C3_SD=stats_phenos_clus$`3`$sd, C4_N= stats_phenos_clus$`4`$n, C4_Min=stats_phenos_clus$`4`$min, C4_Max=stats_phenos_clus$`4`$max, C4_Mean=stats_phenos_clus$`4`$mean, C4_Median=stats_phenos_clus$`4`$median, C4_SD=stats_phenos_clus$`4`$sd)
...
ggplot(ct, aes(as.factor(ct$`K=4`),Bacteroides_fragilis, fill=as.factor(cm$`K=4`))) + geom_violin() + geom_jitter(height = 0, width = 0.1, alpha=0.4) + theme_bw() + xlab("Clusters")  + scale_fill_manual(values =c("firebrick3","gray80","steelblue2","gold3" ))


ggplot(cm, aes(as.factor(cm$`K=4`),tryptophan, fill=as.factor(cm$`K=4`))) + geom_violin() + geom_jitter(height = 0, width = 0.1, alpha=0.4) + theme_bw() + xlab("Clusters")  + scale_fill_manual(values =c("firebrick3","gray80","steelblue2","gold3" ))
...
write.table(my_corr_factors_results, "~/Documents/Project_Metabolome/6.Results/072020/technical_confouders_quantitative.txt", sep = "\t", quote = F)
...
write.table(my_corr_factors_results_bi, "~/Documents/Project_Metabolome/6.Results/072020/technical_confouders_metabolites_prevalence.txt", sep = "\t", quote = F)
...
#write.table(test_mtb_quantitative,"~/Documents/Project_Metabolome/1.Input_files/Models_052020/CLR/Univariate_cc_v3.txt",sep="\t", quote = F, row.names = T)


mtb_quantitative_ibd=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/quant_cc_ibd.txt",sep="\t", header = T, row.names = 1)
mtb_quantitative_diet=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/quant_cc_diet.txt",sep="\t", header = T, row.names = 1)
mtb_quantitative_bugs=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/quant_cc_bugs.txt",sep="\t", header = T, row.names = 1)

#corr_vars=c("host_Sex","host_Age_sampling","run_day_cat","LC.COLUMN","Amount_sample_gram","metabolon_Month_in_freezer")

#Check Correlated dietary factor
cor_diet=cor(mtb_quantitative_diet[,8:151], method = "spearman")
#
highlyCorrelated <- findCorrelation(cor_diet, cutoff=0.85, names = T)
...
for ( i in 1:63) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling") {
    ...
    for (a in 64:ncol(mtb_quantitative_ibd)){
...
mtb_quantitative_bugs2=mtb_quantitative_bugs
#mtb_quantitative_bugs=mtb_quantitative_bugs2
mtb_quantitative_bugs=mtb_quantitative_bugs[,c(1:10,120:ncol(mtb_quantitative_bugs))]
...
for ( i in 1:10) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling"| my_pheno=="ibd_Diagnosis") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 11:ncol(mtb_quantitative_bugs)){
...
my_univariate_ibd2=my_univariate_ibd2[my_univariate_ibd2$phenotype!="ibd_DiseaseLocation",]
my_univariate_ibd2=my_univariate_ibd2[my_univariate_ibd2$phenotype!="ibd_ActiveDisease",]

all_uni_quant=rbind(my_univariate_ibd2, my_univariate_diet)
all_uni_quant=rbind(all_uni_quant, my_univariate_bugs2)
all_uni_quant2=all_uni_quant
all_uni_quant2=subset(all_uni_quant2, all_uni_quant2$phenotype %ni% highlyCorrelated)
all_uni_quant2=subset(all_uni_quant2, all_uni_quant2$phenotype != "Simpson_Index")
all_uni_quant2$FDR=p.adjust(all_uni_quant2$`Pr(>|t|)`,method = "BH")
all_uni_quant2$Bonferroni=p.adjust(all_uni_quant2$`Pr(>|t|)`,method = "bonferroni")


##################
##SAVING RESULTS##
##################
write.table(all_uni_quant2,"~/Documents/Project_Metabolome/6.Results/072020/Results_quantitative_mtb.txt",sep="\t", quote = F, row.names = T)

quant_significant=all_uni_quant2[all_uni_quant2$FDR<0.05,]
#quant_significant=all_uni_quant2[all_uni_quant2$Bonferroni<0.05,]
...
CD=unlist(quant_significant$metabolite[quant_significant$phenotype=="ibd_CD"])
UC=unlist(quant_significant$metabolite[quant_significant$phenotype=="ibd_UC"])
IBD=unlist(quant_significant$metabolite[quant_significant$phenotype=="ibd_Diagnosis"])
...
my_test_list=list(CD =CD, UC=UC,BMI = BMI, Diet = diet,IBD=IBD ,Richness=Richness,Meds=meds)
...
plot_cd=quant_significant[quant_significant$phenotype=="ibd_CD",]
...
ggplot(plot_IBD2, aes( Ruminococcus_gnavus, palmitoylcarnitine__C16,colour=ibd_Diagnosis)) + geom_point() + theme_bw() + geom_smooth(method='lm') + scale_color_manual(values=c("black","firebrick2","blue2"))
...
plot_uc=quant_significant[quant_significant$phenotype=="ibd_UC",]
...
plot_ibdz=quant_significant[quant_significant$phenotype=="ibd_Diagnosis",]

plot_ibdz2=merge(plot_ibdz,annot, by.x = "metabolite", by.y = "clean")
plot_ibdz2$dir=-1
plot_ibdz2[plot_ibdz2$Estimate>0,]$dir=1

ggplot(plot_ibdz2, aes(SUB.PATHWAY,dir, fill=as.factor(dir))) + geom_bar(stat = "identity") + coord_flip() + scale_fill_manual(values=c("blue2", "red3")) + theme_bw()
...
#write.table(test_mtb_presence,"~/Documents/Project_Metabolome/1.Input_files/Models_052020/Univariate_cc_prev.txt",sep="\t", quote = F, row.names = T)

mtb_prev_ibd=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/prev_cc_ibd.txt",sep="\t", header = T, row.names = 1)
mtb_prev_diet=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/prev_cc_diet.txt",sep="\t", header = T, row.names = 1)
mtb_prev_bugs=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/prev_cc_bugs.txt",sep="\t", header = T, row.names = 1)
...
for ( i in 1:10) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling"| my_pheno=="ibd_Diagnosis") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 11:ncol(mtb_prev_bugs)){
...
prev_univariate_ibd2=prev_univariate_ibd2[prev_univariate_ibd2$phenotype!="ibd_DiseaseLocation",]
prev_univariate_ibd2=prev_univariate_ibd2[prev_univariate_ibd2$phenotype!="ibd_ActiveDisease",]
...
all_uni_prev2=all_uni_prev
all_uni_prev2=subset(all_uni_prev2, all_uni_prev2$phenotype %ni% highlyCorrelated)
all_uni_prev2=subset(all_uni_prev2, all_uni_prev2$phenotype != "Simpson_Index")
all_uni_prev2$FDR=p.adjust(all_uni_prev2$`Pr(>|z|)`,method = "BH")
all_uni_prev2$Bonferroni=p.adjust(all_uni_prev2$`Pr(>|z|)`,method = "bonferroni")
...
write.table(all_uni_prev2,"~/Documents/Project_Metabolome/6.Results/072020/Results_prevalence.txt",sep="\t", quote = F, row.names = T)

#prev_significant=all_uni_prev2[all_uni_prev2$FDR<0.05,]
prev_significant=all_uni_prev2[all_uni_prev2$Bonferroni<0.05,]
...
CD=unlist(prev_significant$metabolite[prev_significant$phenotype=="ibd_CD"])
UC=unlist(prev_significant$metabolite[prev_significant$phenotype=="ibd_UC"])
IBD=unlist(prev_significant$metabolite[prev_significant$phenotype=="ibd_Diagnosis"])
...
my_test_list=list(CD =CD, UC=UC,BMI = BMI, IBD=IBD, Diet = diet, Bugs=Bugs ,Meds=meds, Richness=Richness)
upset(fromList(my_test_list), order.by = "freq", nsets = 7)
...
only_ibd_quant=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/ibd_quant.txt",sep="\t", header = T, row.names = 1)
...
for ( i in 1:237) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 238:ncol(only_ibd_quant)){
...
ibd_only_res_clean2=ibd_only_res_clean
ibd_only_res_clean2=subset(ibd_only_res_clean2, ibd_only_res_clean2$phenotype %ni% highlyCorrelated)
ibd_only_res_clean2=subset(ibd_only_res_clean2, ibd_only_res_clean2$phenotype != "Simpson_Index")
ibd_only_res_clean2$FDR=p.adjust(ibd_only_res_clean2$`Pr(>|t|)`,method = "BH")
ibd_only_res_clean2$Bonferroni=p.adjust(ibd_only_res_clean2$`Pr(>|t|)`,method = "bonferroni")
...
###
###
write.table(ibd_only_res_clean2,"~/Documents/Project_Metabolome/6.Results/072020/Results_lm_onlyIBD.txt",sep="\t", quote = F, row.names = T)

ibd_only_sig=ibd_only_res_clean2[ibd_only_res_clean2$FDR<0.05,]
ibd_only_sig=ibd_only_res_clean2[ibd_only_res_clean2$Bonferroni<0.05,]
...
my_test_list=list(CD_vs_UC =CD_vs_UC, Bowel_move=Bowel_move, Resection = Resection, Diet = diet ,Meds=meds, Richness=Richness,Ileocecal_valve=Ileocecal_valve,Num_resections=Num_resections,Active_disease=Active_disease)
...
only_ibd_quant=read.table("~/Documents/Project_Metabolome/6.Results/072020/input/ibd_prev.txt",sep="\t", header = T, row.names = 1)
only_ibd_quant$ibd_CurrentStomaOrPouch=NULL
only_ibd_quant$host_Sex.1=NULL
only_ibd_quant$med_parasympathicolytic_inhaler=NULL
only_ibd_quant$med_ranitidine=NULL
...
ibd_only_results_prevalence=my_univariate_results

#ibd_only_results_prevalence=ibd_only_results_prevalence[ibd_only_results_prevalence$phenotype!="X15_2_anacardic_acid",]
#ibd_only_results_prevalence=ibd_only_results_prevalence[ibd_only_results_prevalence$phenotype!="X15_3_anacardic_acid",]
#ibd_only_results_prevalence=ibd_only_results_prevalence[ibd_only_results_prevalence$phenotype!="X3_5_adenylylcytidine",]
#ibd_only_results_prevalence=ibd_only_results_prevalence[ibd_only_results_prevalence$phenotype!="X3_5_adenylylguanosine.",]

#ibd_only_results_prevalence$FDR=p.adjust(ibd_only_results_prevalence$`Pr(>|z|)`,method = "BH")
#ibd_only_results_prevalence$Bonferroni=p.adjust(ibd_only_results_prevalence$`Pr(>|z|)`,method = "bonferroni")

ibd_only_results_prevalence2=ibd_only_results_prevalence
ibd_only_results_prevalence2=subset(ibd_only_results_prevalence2, ibd_only_results_prevalence2$phenotype %ni% highlyCorrelated)
ibd_only_results_prevalence2=subset(ibd_only_results_prevalence2, ibd_only_results_prevalence2$phenotype != "Simpson_Index")
ibd_only_results_prevalence2$FDR=p.adjust(ibd_only_results_prevalence2$`Pr(>|z|)`,method = "BH")
ibd_only_results_prevalence2$Bonferroni=p.adjust(ibd_only_results_prevalence2$`Pr(>|z|)`,method = "bonferroni")


write.table(ibd_only_results_prevalence2,"~/Documents/Project_Metabolome/6.Results/072020/Results_onlyIBD_prev.txt",sep="\t", quote = F, row.names = T)


test_pv=as.data.frame(table(only_ibd_quant$host_SmokeCurrentSmoker, only_ibd_quant$cotinine))
setDT(test_pv)[,prop:=Freq/sum(Freq),by=Var1]
ggplot(test_pv, aes(Var1,prop, fill=Var2)) + geom_bar(stat = "identity") + theme_bw() + ylab ("Cotinine") + xlab ("Current smoking")
...
cc_pheno4$Row.names=NULL
cc_pheno5=merge(my_batch_id,cc_pheno4, by="row.names")
my_regress_phenos=cc_pheno5[,c("Row.names","run_day_cat", "Amount_sample_gram", "LC.COLUMN" , "metabolon_Month_in_freezer" , "host_Age_sampling", "host_BMI","host_Sex", "ibd_Diagnosis")]
rownames(my_regress_phenos)=my_regress_phenos$Row.names
my_regress_phenos$Row.names=NULL
...
n=1
for (i in 7:ncol(rgs_tx)){
  my_lm=lm(rgs_tx[,i] ~ ibd_Diagnosis,data = rgs_tx)
  rgs_tx2[,n] = my_lm$residuals
  n=n+1
}
rgs_tx2=as.data.frame(rgs_tx2)
rownames(rgs_tx2)=rgs_tx$Row.names
colnames(rgs_tx2)=colnames(rgs_tx)[7:ncol(rgs_tx)]


library ("Hmisc")
test_corr2=rcorr(as.matrix(test_corr), type = "spearman")
test_corr_coef=test_corr2$r
test_corr_coef_sel=test_corr_coef[(ncol(rgs_mb4)+1):nrow(test_corr_coef), 1:ncol(rgs_mb4)]
test_corr_pval=test_corr2$P
test_corr_pval_sel=test_corr_pval[(ncol(rgs_mb4)+1):nrow(test_corr_pval), 1:ncol(rgs_mb4)]
...
colnames(corr_coef)=c("BGC","Metabolites","Coeff")
colnames(corr_pval)=c("BGC2","Metabolites2","P_val")
bcg_metabolites=cbind(corr_coef, corr_pval)



flag=1
for (mtb in 1:ncol(rgs_mb2)){
  for (tx in 1:ncol(rgs_tx2)){
    mmtb=colnames(rgs_mb2)[mtb]
    mtx=colnames(rgs_tx2)[tx]
    mest=cor.test(rgs_mb2[,mtb],rgs_tx2[,tx], method = "spearman")$estimate
    mpval=cor.test(rgs_mb2[,mtb],rgs_tx2[,tx], method = "spearman")$p.value
    mtb_tx=data.frame(Metabolite=mmtb,Taxa=mtx,Coefficient=mest,P_val=mpval)
    if (flag==1){
      res_mtb_tx=mtb_tx
      flag=7
    } else{
      res_mtb_tx=rbind(res_mtb_tx,mtb_tx)
    }
  }
}
...
rgs_mb3$run_day_cat=NULL
rgs_mb3$Amount_sample_gram=NULL
rgs_mb3$LC.COLUMN=NULL
rgs_mb3$metabolon_Month_in_freezer=NULL

rgs_tx3$run_day_cat=NULL
rgs_tx3$Amount_sample_gram=NULL
rgs_tx3$LC.COLUMN=NULL
rgs_tx3$metabolon_Month_in_freezer=NULL
rgs_tx3$ibd_Diagnosis=NULL

mgtx=merge(rgs_mb3, rgs_tx3, by="Row.names")

mgtxControl=subset(mgtx,mgtx$ibd_Diagnosis=="Control")
mgtxCD=subset(mgtx,mgtx$ibd_Diagnosis=="CD")
mgtxUC=subset(mgtx,mgtx$ibd_Diagnosis=="UC")

flag=1
for (mtb in 3:856){
  for (tx in 857:ncol(mgtxControl)){
    mmtb=colnames(mgtxControl)[mtb]
    mtx=colnames(mgtxControl)[tx]
    
    mest=cor.test(mgtxControl[,mtb],mgtxControl[,tx], method = "spearman")$estimate
    mpval=cor.test(mgtxControl[,mtb],mgtxControl[,tx], method = "spearman")$p.value
    
    mestcd=cor.test(mgtxCD[,mtb],mgtxCD[,tx], method = "spearman")$estimate
    mpvalcd=cor.test(mgtxCD[,mtb],mgtxCD[,tx], method = "spearman")$p.value
    
    mestuc=cor.test(mgtxUC[,mtb],mgtxUC[,tx], method = "spearman")$estimate
    mpvaluc=cor.test(mgtxUC[,mtb],mgtxUC[,tx], method = "spearman")$p.value
    
    mtb_tx2=data.frame(Metabolite=mmtb,Taxa=mtx,Coefficient=mest,P_val=mpval, Coefficient_CD=mestcd,P_val_CD=mpvalcd,Coefficient_UC=mestuc,P_val_UC=mpvaluc)
    if (flag==1){
      res_mtb_tx2=mtb_tx2
      flag=7
    } else{
      res_mtb_tx2=rbind(res_mtb_tx2,mtb_tx2)
    }
  }
}


res_mtb_tx$sig="white"
res_mtb_tx$FDR=p.adjust(res_mtb_tx$P_val, method = "BH")



res_mtb_tx[res_mtb_tx$Coefficient > 0.1 & res_mtb_tx$FDR < 0.05, ]$sig="salmon"
res_mtb_tx[res_mtb_tx$Coefficient > 0.2 & res_mtb_tx$FDR < 0.05, ]$sig="red"
res_mtb_tx[res_mtb_tx$Coefficient > 0.3 & res_mtb_tx$FDR < 0.05, ]$sig="red2"
res_mtb_tx[res_mtb_tx$Coefficient > 0.4 & res_mtb_tx$FDR < 0.05, ]$sig="red4"

res_mtb_tx[res_mtb_tx$Coefficient < -0.1 & res_mtb_tx$FDR < 0.05, ]$sig="lightblue"
res_mtb_tx[res_mtb_tx$Coefficient < -0.2 & res_mtb_tx$FDR < 0.05, ]$sig="blue"
res_mtb_tx[res_mtb_tx$Coefficient < -0.3 & res_mtb_tx$FDR < 0.05, ]$sig="blue2"
res_mtb_tx[res_mtb_tx$Coefficient < -0.4 & res_mtb_tx$FDR < 0.05, ]$sig="blue4"

res_mtb_tx$Coef2=res_mtb_tx$Coefficient
res_mtb_tx$Coef2[res_mtb_tx$FDR > 0.05]=0
...
#pheatmap(t(my_test2), cluster_cols = T, cluster_rows = T,show_rownames = T, show_colnames = T, annotation_col = annot_v2, scale = "none", color = inferno(20),fontsize = 5)
...
#bgc=read.table("~/Documents/Project_Metabolome/7.Bacterial_genes_and_pathways/all/b1_core_rpkm_filt_renamed.txt", sep = "\t", row.names = 1, stringsAsFactors = F, header = T, check.names = F )

#awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_LLD.txt b1_core_rpkm_filt.txt | tr " " "\t" > b1_core_rpkm_filt_renamed.txt

#awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_LLD.txt b2_core_rpkm_filt.txt | tr " " "\t" > b2_core_rpkm_filt_renamed.txt

#awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_LLD.txt b3_core_rpkm_filt.txt | tr " " "\t" > b3_core_rpkm_filt_renamed.txt

#awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_LLD.txt b4_core_rpkm_filt.txt | tr " " "\t" > b4_core_rpkm_filt_renamed.txt

#awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_IBD.txt b5_core_rpkm_filt.txt | tr " " "\t" > b5_core_rpkm_filt_renamed.txt 

# awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_IBD.txt b6_core_rpkm_filt.txt | tr " " "\t" > b6_core_rpkm_filt_renamed.txt

# awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_IBD.txt b7_core_rpkm_filt.txt | tr " " "\t" > b7_core_rpkm_filt_renamed.txt

# awk 'NR==FNR{d[$1]=$2;next}FNR==1{for(i=1;i<=NF;i++)$i=d[$i]?d[$i]:$i}7' ./rename_IBD.txt b8_core_rpkm_filt.txt | tr " " "\t" > b8_core_rpkm_filt_renamed.txt


setwd("~/Documents/Project_Metabolome/7.Bacterial_genes_and_pathways/all/")
...
features_per_samples=data.frame(Sample=colnames(bgc), Counts=colSums(bgc!=0))
samples_per_feature=data.frame(BCG=rownames(bgc), Counts=rowSums(bgc!=0))

ggplot(samples_per_feature, aes(reorder(BCG,-Counts), Counts )) + geom_bar(stat = "identity", fill="lightblue3") + theme_classic() + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + ylab ("Number of Biosynthetic genes clusters")  + xlab ("Samples")
...
#Remove samples with less 100 BioSynthetic Gene Clusters (n=9)
remove_samples=row.names(features_per_samples)[features_per_samples$Counts <99]
bgc_filter=bgc[, colnames(bgc) %ni%  remove_samples]

#Filtering prevalence in 20% => 568 metabolites
bcg_fil_trans=transform_and_filter_taxa(bgc_filter, samples_row = F, method = "clr",missing_filter = 20)
...
#clus_taxa
bgc_rgs=merge(my_regress_phenos,bcg_fil_trans, by="row.names")
bgc_rgs2=matrix(nrow=nrow(bgc_rgs), ncol = ncol(bcg_fil_trans))
n=1
for (i in 7:ncol(bgc_rgs)){
  my_lm=lm(bgc_rgs[,i] ~ ibd_Diagnosis,data = bgc_rgs)
  bgc_rgs2[,n] = my_lm$residuals
  n=n+1
}
bgc_rgs2=as.data.frame(bgc_rgs2)
rownames(bgc_rgs2)=bgc_rgs$Row.names
colnames(bgc_rgs2)=colnames(bgc_rgs)[7:ncol(bgc_rgs)]


rgs_mb4=rgs_mb3[rgs_mb3$Row.names %in% row.names(bgc_rgs2),]

row.names(rgs_mb4)=rgs_mb4$Row.names
rgs_mb4$Row.names=NULL
rgs_mb4$ibd_Diagnosis=NULL

write.table(rgs_mb4, "~/Documents/Project_Metabolome/7.Bacterial_genes_and_pathways/mtb.txt", sep = "\t", quote = F)
write.table(bgc_rgs2, "~/Documents/Project_Metabolome/7.Bacterial_genes_and_pathways/bgc.txt", sep = "\t", quote = F)

test_corr=merge(rgs_mb4,bgc_rgs2, by="row.names")
rownames(test_corr)=test_corr$Row.names
test_corr$Row.names=NULL
...
colnames(corr_pval)=c("BGC2","Metabolites2","P_val")
bcg_metabolites=cbind(corr_coef, corr_pval)
bcg_metabolites$BGC2=NULL
bcg_metabolites$Metabolites2=NULL
bcg_metabolites$Coeff2=bcg_metabolites$Coeff
bcg_metabolites$FDR=p.adjust(bcg_metabolites$P_val, method = "BH")
bcg_metabolites[bcg_metabolites$FDR>0.05,]$Coeff2=0

bcg_metabolites_plot=dcast(bcg_metabolites, formula=Metabolites~BGC, value.var="Coeff2")
row.names(bcg_metabolites_plot)=bcg_metabolites_plot$Metabolites
bcg_metabolites_plot$Metabolites=NULL

pheatmap(bcg_metabolites_plot, cluster_cols = T, cluster_rows = T,show_rownames = F, show_colnames = F, breaks = seq(-1,1,length.out=60),scale = "none",fontsize = 5, color = colorRampPalette(c("navy", "white", "firebrick3"))(60))
...
###################################
rownames(rgs_tx3)=rgs_tx3$Row.names
rgs_tx3$Row.names=NULL

test_corr_tx=merge(rgs_tx3,bgc_rgs2, by="row.names")
rownames(test_corr_tx)=test_corr_tx$Row.names
test_corr_tx$Row.names=NULL

library ("Hmisc")
test_corr_tx2=rcorr(as.matrix(test_corr_tx), type = "spearman")
test_corr_tx_coef=test_corr_tx2$r
test_corr_tx_coef_sel=test_corr_tx_coef[(ncol(rgs_tx3)+1):nrow(test_corr_tx_coef), 1:ncol(rgs_tx3)]
test_corr_tx_pval=test_corr_tx2$P
test_corr_tx_pval_sel=test_corr_tx_pval[(ncol(rgs_tx3)+1):nrow(test_corr_tx_pval), 1:ncol(rgs_tx3)]

corr_coef_tx=melt(test_corr_tx_coef_sel)
corr_pval_tx=melt(test_corr_tx_pval_sel)

table(corr_coef_tx$Var1 == corr_pval_tx$Var1)
table(corr_coef_tx$Var2 == corr_pval_tx$Var2)
...
bcg_tx_plot=dcast(bcg_tx, formula=Metabolites~BGC, value.var="Coeff2")
