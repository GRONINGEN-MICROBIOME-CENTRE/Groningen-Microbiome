...
[REMOVE: Implemented below after filtering]Normalize and impute missing metabolites
---
...
#write.table(q_mtb, "~/Documents/Project_Metabolome/1.Input_files/Imputed_metabolites", sep = "\t",quote = F)
...
my_corr_factors_results=matrix(ncol = 5, nrow = 4626)
b=1
...
colnames(my_corr_factors_results)=c("Factor", "Metabolite","Estimate","p.value")
...
my_corr_factors_results_bi=matrix(ncol = 5, nrow = 4626)
b=1
for ( a in 1:ncol(my_correction_factors)){
...
write.table(test_mtb_quantitative,"~/Documents/Project_Metabolome/1.Input_files/Univariate_cc.txt",sep="\t", quote = F, row.names = T)

##REMOVE DIET & IBD PHENOS##
mtb_quantitative_ibd=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_032020/Univariate_quant_ibd_meds.txt",sep="\t", header = T, row.names = 1)
mtb_quantitative_diet=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_032020/Univariate_quant_diet.txt",sep="\t", header = T, row.names = 1)
mtb_quantitative_bugs=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_032020/Univariate_quant_bugs.txt",sep="\t", header = T, row.names = 1)
...
for ( i in 1:130) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling"| my_pheno=="ibd_Diagnosis") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 131:ncol(mtb_quantitative_bugs)){
...
Bugs=unlist(quant_significant$metabolite[quant_significant$phenotype%in%colnames(taxa_filt)])
BMI=unlist(quant_significant$metabolite[quant_significant$phenotype=="host_BMI"])

my_test_list=list(CD =CD, UC=UC,BMI = BMI, Diet = diet, Bugs=Bugs ,Meds=meds)
upset(fromList(my_test_list), order.by = "freq", nsets = 6)
...
as.formula(paste(my_trait, paste(corr_vars, collapse = " + "), sep = " ~ "))
