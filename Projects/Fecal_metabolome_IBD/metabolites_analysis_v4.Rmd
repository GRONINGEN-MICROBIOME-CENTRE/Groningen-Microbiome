...
write.table(q_mtb, "~/Documents/Project_Metabolome/1.Input_files/Imputed_metabolites_clr_v4", sep = "\t",quote = F)
...
clus_mtb=read.table("./clst_mtb2", sep = "\t", header = T, row.names = 1)
clus_taxa=read.table("./clst_taxa2", sep = "\t", header = T, row.names = 1)
clus_pheno=read.table("./clst_pheno2", sep = "\t", header = T, row.names = 1)
...
# Warning k=k+1
#plot shows that 4 is the best cluster solution (k=5)
k=4 
...
ggplot (new_pc_bac, aes(PC1,PC2, color=as.factor(new_pc_bac$`K=5`))) + geom_point() + theme_bw() + scale_color_manual(values=c("black","firebrick2", "lightblue2", "gold2","purple"))
...
ggplot (new_pc_mtb, aes(PC1,PC2, color=as.factor(new_pc_bac$`K=5`))) + geom_point() + theme_bw() + scale_color_manual(values=c("black","firebrick2", "lightblue2", "gold2","purple"))
...
plot_ly(x=new_pc_mtb$PC1,y=new_pc_mtb$PC2,z=new_pc_mtb$PC3, color = new_pc_mtb$`K=5`, colors = c("black","firebrick2", "lightblue2", "gold2","purple"))


ph_heat=new_pc_mtb[,c("Row.names","ibd_Diagnosis","ibd_DiseaseLocation","K=5")]
...
htmtb=melt(htmtb,id.vars=c("Row.names","ibd_Diagnosis","ibd_DiseaseLocation","K=5"))
htbac=melt(htbac,id.vars=c("Row.names","ibd_Diagnosis","ibd_DiseaseLocation","K=5"))
...
s1=new_pc_mtb[,c("Row.names","K=5")]
rownames(s1)=s1$Row.names
s1$Row.names=NULL
s1=merge(s1,clus_mtb, by="row.names")
s1=melt(s1, id.vars=c("Row.names","K=5"))
s2=new_pc_mtb[,c("Row.names","K=5","ibd_Diagnosis","ibd_DiseaseLocation", "ibd_ActiveDisease","clinical_BowelMovementADayDef", "Shannon_Index")]
s2=melt(s2, id.var=c("Row.names","K=5"))
...
new_pc_mtb2=new_pc_mtb[,c("Row.names","K=5","ibd_Diagnosis","ibd_DiseaseLocation", "ibd_ActiveDisease","clinical_BowelMovementADayDef", "Shannon_Index")]
rownames(new_pc_mtb2)=new_pc_mtb2$Row.names
new_pc_mtb2$Row.names=NULL
new_pc_mtb2$cluster=paste("c", new_pc_mtb2$`K=5`,sep="_")
new_pc_mtb2$`K=5`=NULL
...
pheatmap(t(new_clus_mtb3), cluster_cols = F, cluster_rows = F,show_rownames = T, gaps_col= c(head(as.numeric(cumsum(table(new_pc_mtb2$cluster)))), -1),show_colnames = F, annotation_col = new_pc_mtb2, scale = "none", color = inferno(20),fontsize = 5, annotation_colors = list ( ibd_Diagnosis=c("CD"="firebrick2", "UC"="blue2", "Control"="white", "IBDU"="black"),cluster=c("c_1"="black","c_2"="firebrick2","c_3"="lightblue2","c_4"="gold2","c_5"="purple" )))
...
write.table(my_corr_factors_results, "~/Documents/Project_Metabolome/6.Results/052020_CLR/confouders_quantitative_v3", sep = "\t", quote = F)
...
write.table(my_corr_factors_results_bi, "~/Documents/Project_Metabolome/6.Results/052020_CLR/confouders_prevalence_v3.txt", sep = "\t", quote = F)
...
mtb_quantitative_ibd=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/CLR/quant_cc_ibd.txt",sep="\t", header = T, row.names = 1)
mtb_quantitative_diet=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/CLR/quant_cc_diet.txt",sep="\t", header = T, row.names = 1)
mtb_quantitative_bugs=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/CLR/quant_cc_bugs.txt",sep="\t", header = T, row.names = 1)
...
for ( i in 1:119) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling"| my_pheno=="ibd_Diagnosis") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 120:ncol(mtb_quantitative_bugs)){
...
mtb_prev_ibd=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/prev_cc_ibd.txt",sep="\t", header = T, row.names = 1)
mtb_prev_diet=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/prev_cc_diet.txt",sep="\t", header = T, row.names = 1)
mtb_prev_bugs=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/prev_cc_bugs.txt",sep="\t", header = T, row.names = 1)
...
for ( i in 1:119) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling"| my_pheno=="ibd_Diagnosis") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 120:ncol(mtb_prev_bugs)){
...
write.table(all_uni_prev,"~/Documents/Project_Metabolome/6.Results/Results_prevalence_052020.txt",sep="\t", quote = F, row.names = T)
...
only_ibd_quant=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/CLR/ibd_quant.txt",sep="\t", header = T, row.names = 1)
...
for ( i in 1:346) {
  ...
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 347:ncol(only_ibd_quant)){
...
ibd_only_res_clean=ibd_only_res_clean[ibd_only_res_clean$metabolite!="Veillonella_parvula",]
ibd_only_res_clean=ibd_only_res_clean[ibd_only_res_clean$metabolite!="Parasutterella_excrementihominis",]
ibd_only_res_clean=ibd_only_res_clean[ibd_only_res_clean$metabolite!="Bilophila_wadsworthia",]
ibd_only_res_clean=ibd_only_res_clean[ibd_only_res_clean$metabolite!="Escherichia_coli",]
ibd_only_res_clean=ibd_only_res_clean[ibd_only_res_clean$metabolite!="Akkermansia_muciniphila",]
...
write.table(ibd_only_res_clean,"~/Documents/Project_Metabolome/6.Results/052020_CLR/Results_onlyIBD_052020.txt",sep="\t", quote = F, row.names = T)
...
only_ibd_quant=read.table("~/Documents/Project_Metabolome/1.Input_files/Models_052020/ibd_prev.txt",sep="\t", header = T, row.names = 1)
only_ibd_quant$ibd_CurrentStomaOrPouch=NULL
only_ibd_quant$host_Sex.1=NULL
only_ibd_quant$med_parasympathicolytic_inhaler=NULL
only_ibd_quant$med_ranitidine=NULL
only_ibd_quant=only_ibd_quant[,1:860]
flag=1
for ( i in 1:346) {
  my_pheno=colnames(only_ibd_quant)[i]
  if (my_pheno=="run_day_cat" | my_pheno=="LC.COLUMN"| my_pheno=="Amount_sample_gram"| my_pheno=="metabolon_Month_in_freezer" | my_pheno=="host_Sex" | my_pheno=="host_Age_sampling") {
    ...
    print (paste("Testing:",my_pheno))
    for (a in 427:ncol(only_ibd_quant)){
      my_trait=colnames(only_ibd_quant)[a]
      my_uni_test=only_ibd_quant[,c("host_Sex","host_Age_sampling","run_day_cat","LC.COLUMN","Amount_sample_gram","metabolon_Month_in_freezer","ibd_Diagnosis",my_pheno,my_trait)]
      my_preds=c("host_Sex","host_Age_sampling","run_day_cat","LC.COLUMN","Amount_sample_gram","metabolon_Month_in_freezer","ibd_Diagnosis",my_pheno)
      my_f=as.formula(paste(my_trait, paste(my_preds, collapse = " + "), sep = " ~ "))
...
my_pred = predict(model,testing,type="prob")
confusionMatrix(data=my_pred_class, testing$ibd_Diagnosis)
my_roc=pROC::roc(testing$ibd_Diagnosis,my_pred[["CD"]], auc=T, ci=T, smooth=F, levels=c("a_nonCD", "CD"))
...
rf_IBD=rf_IBD[,c(4,70:ncol(rf_IBD))]
levels(rf_IBD$ibd_Diagnosis) <- c(levels(rf_IBD$ibd_Diagnosis), "IBD", "a_nonCD", "a_nonUC") 
rf_IBD[rf_IBD$ibd_Diagnosis!="UC",]$ibd_Diagnosis="a_nonUC"
rf_IBD$ibd_Diagnosis=droplevels(rf_IBD$ibd_Diagnosis)
...
my_pred = predict(model,testing,type="prob")
confusionMatrix(data=my_pred_class, testing$ibd_Diagnosis)
my_roc=pROC::roc(testing$ibd_Diagnosis,my_pred[["UC"]], auc=T, ci=T, smooth=F, levels=c("a_nonUC", "UC"))
...
write.table(rgs_tx2,"./Documents/Project_Metabolome/1.Input_files/mmvec/taxa.txt", sep="\t", quote = F)
write.table(rgs_mb2,"./Documents/Project_Metabolome/1.Input_files/mmvec/mtb.txt", sep="\t", quote = F)
