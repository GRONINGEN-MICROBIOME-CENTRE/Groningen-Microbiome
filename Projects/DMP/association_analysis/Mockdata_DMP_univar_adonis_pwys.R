...
#   - this is slightly different implementation from DMP_do_univar_adonis_pwys.R code 
...
inDF <- read.table('heritability_analysis_v2/mock_data/DMP_mock_microbiome_pwys_filtered.csv',sep=',',header=T)
...
write.table(adonisResults,paste0(outFolder,"/mockdata_adonis_Pwys_results_table.csv"),sep=",",row.names=F)
...
# === LOAD STEP 1 RESULTS ===
adonisResults1 <- read.table(paste0(outFolder,"/mockdata_adonis_Pwys_results_table.csv"),sep=",",header=T,stringsAsFactors = F)
...
for (ug in unique(adonisRes1grp$Group.smaller)) {
  ...
  if (length(sigPhenos) ==0) {
    ...
    # grab pathways for Bray-Curtis calculation
