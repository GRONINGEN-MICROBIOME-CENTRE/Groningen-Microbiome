...
```{r 2.1.1, eval=FALSE}
lld_vsv_exp_lm_adjAbun_res<-lm_btw_mats_adjAbun2(vsgv_lld,lld_exp,lld_basic,covar,lld_abun_clr,info, 9)
...
```{r 2.1.2, eval=FALSE}
## Prepare covariate table
lld_dsv_exp_lr_adjAbun_res<-lr_btw_mats_adjAbun2(dsgv_lld,lld_exp,lld_basic,covar,lld_abun_clr,info,9)
...
vsv_ba_lm_res.sig.anno.edge<-read.table("09.Microbial_GWAS/vsv_ba_lm_adjAbun.sig.anno.tsv",
...
lld_vsv_exp_lm_adjAbun.edge <- lld_vsv_exp_lm_adjAbun_res
lld_vsv_exp_lm_adjAbun.edge$Y<-as.character(lld_vsv_exp_lm_adjAbun.edge$Y)
lld_vsv_exp_lm_adjAbun.edge$X<-as.character(lld_vsv_exp_lm_adjAbun.edge$X)

lld_ba_exp_lm_adjAbun.edge  <- lld_ba_exp_lm_res
lld_ba_exp_lm_adjAbun.edge$Y<-as.character(lld_ba_exp_lm_adjAbun.edge$Y)
lld_ba_exp_lm_adjAbun.edge$X<-as.character(lld_ba_exp_lm_adjAbun.edge$X)
...
for (i in exp_id) {
  ...
  for (j in vsv_id) {
    for (k in ba_id) {
      ...
      p_vsv_exp <- lld_vsv_exp_lm_adjAbun.edge$p[lld_vsv_exp_lm_adjAbun.edge$X==i & lld_vsv_exp_lm_adjAbun.edge$Y==j]
      p_ba_exp  <- lld_ba_exp_lm_adjAbun.edge$p[  lld_ba_exp_lm_adjAbun.edge$X==i & lld_ba_exp_lm_adjAbun.edge$Y==k]
...
```{r 3.2, eval=FALSE}
## load SV-BA associations restuls
dsv_ba_lm_res.sig.anno.edge<-read.table("09.Microbial_GWAS/dsv_ba_lm_adjAbun.sig.anno.tsv",
                                        sep = "\t", header = T,stringsAsFactors = F,check.names = F, quote = "")

load("11.Causal_inference/RData/lld_dsv_exp_lr_adjAbun_res.RData")
load("11.Causal_inference/RData/lld_ba_exp_lm_res.RData")

lld_dsv_exp_lr_adjAbun.edge <- lld_dsv_exp_lr_adjAbun_res
lld_dsv_exp_lr_adjAbun.edge$Y<-as.character(lld_dsv_exp_lr_adjAbun.edge$Y)
lld_dsv_exp_lr_adjAbun.edge$X<-as.character(lld_dsv_exp_lr_adjAbun.edge$X)
...
for (i in exp_id) {
  ...
  for (j in dsv_id) {
    for (k in ba_id) {
      ...
      p_dsv_exp <- lld_dsv_exp_lr_adjAbun.edge$p[lld_dsv_exp_lr_adjAbun.edge$Y==i & lld_dsv_exp_lr_adjAbun.edge$X==j]
      p_ba_exp  <- lld_ba_exp_lm_adjAbun.edge$p[  lld_ba_exp_lm_adjAbun.edge$X==i & lld_ba_exp_lm_adjAbun.edge$Y ==k]
...
```{r 5.2}
mediation_m1<-read.table("11.Causal_inference/mediation.tsv", 
                            sep = "\t", header = T, stringsAsFactors = F) %>% .[.$MediationDirection !="none",]
mediation_m2<-read.table("11.Causal_inference/mediation_model2.tsv", 
                            sep = "\t", header = T, stringsAsFactors = F) %>% .[.$MediationDirection !="none",]
mediation_m1$Model<-1
mediation_m2$Model<-2

mediation_merge<-rbind(mediation_m2, mediation_m1)
mediation_merge<-mediation_merge[!duplicated(paste(mediation_merge$indv,mediation_merge$dv1,mediation_merge$dv2,mediation_merge$MediationDirection)),]
mediation_merge<-mediation_merge[!duplicated(paste(mediation_merge$indv,mediation_merge$dv1,mediation_merge$dv2)),]

mediation_final<-mediation_merge


write.table(mediation_final, "11.Causal_inference/mediation_final_merged.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)
## venn diagram
...
```{r 5.3}
mediation_final<-read.table("11.Causal_inference/mediation_final_merged.tsv", 
                            sep = "\t", header = T, stringsAsFactors = F)
