...
Meta_analyze = function(Gender, Results, BMI=F){
    if (Gender == "Male"){
      ...
      if (BMI == F){ Results_Rotter = read_csv("Metanalysis/Rotterdam_2021_2/TMAO_RS16s_26April2021_MALES.csv")
      } else {Results_Rotter = read_csv("Metanalysis/Rotterdam_2021_2/TMAO_RS16s_BMI_26April2021_MALES.csv") }
    
    } else if (Gender == "Female"){
      Results %>% filter(grepl("\\.1\\.",Source)) %>% select(-c(FDR, Source)) -> Results_LLD
      if (BMI == F){ Results_Rotter = read_csv("Metanalysis/Rotterdam_2021_2/TMAO_RS16s_26April2021_FEMALES.csv")
      } else {Results_Rotter = read_csv("Metanalysis/Rotterdam_2021_2/TMAO_RS16s_BMI_26April2021_FEMALES.csv") }
      
    } else{
      Results %>% filter(grepl("\\.3\\.",Source)) %>% select(-c(FDR, Source)) -> Results_LLD
      if (BMI == F){ Results_Rotter = read_csv("Metanalysis/Rotterdam_2021_2/TMAO_RS16s_All_26April2021.csv")
      } else {Results_Rotter = read_csv("Metanalysis/Rotterdam_2021_2/TMAO_RS16s_All_BMI_26April2021.csv") }
    }
  ...
  Results_LLD %>% ggplot() + geom_point(aes(x=Beta, y=-log10(Pvalue), shape =Metabolite, col = Pvalue<0.05 )) +
    facet_grid(Metabolite~Level, scales="free") + theme_bw() + ggtitle('LLD') -> Summary_LLD
  Results_Rotter %>% ggplot() + geom_point(aes(x=Beta, y=-log10(Pvalue), shape =Metabolite, col = Pvalue<0.05)) +
  ...
  Summary_LLD + Summary_rot + plot_layout(guides = "collect") -> Summary_cohorts
    ...
    Meta_stats %>% filter(meta_FDR < 0.07) -> Significant
    Final_figure = NULL
    for (B in unique(Significant$Bug)){
      Significant %>% filter(Bug == B) -> Taxa
      if (dim(Taxa)[1] > 1){
        print(B)
        ggplot(Taxa) + geom_bar(aes_(y=Taxa$Treatment_effect,x=Taxa$Metabolite), stat="identity") + theme_bw() +ggtitle(B) +theme(axis.title.x=element_blank(), axis.ticks.x=element_blank()) +ylab("Effect") -> Fig
...
unique(Hetero_results) %>% arrange(P) %>% mutate(FDR= p.adjust(P, "fdr"))
###################################
