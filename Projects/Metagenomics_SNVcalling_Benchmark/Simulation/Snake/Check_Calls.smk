...
rule Find_Covered_variants:
	input:
		Polymor = Working_dir +"Simulation/Polymorphisms_formatted.tsv",
...
rule ROC:
        ...
        run:    
                ...
                for Threshold in range(0,5000, 20): 
                        ...
                        shell("ml Biopython/1.65-foss-2015b-Python-3.4.1 ; python scripts/Classify_Variants.py --ref {input.Covered} --calls {output.tempcall} --covered {input.coverage} --output {temporal_output2} --threshold {Threshold}")
                ...
                for F in list_files:
                        ...
                        with open(F) as FILE:
                                ...
                                for line in FILE:
                                        ...
                                        l = line.rstrip().split()
                                        try :TPR = float(l[0])/(float(l[0])+float(l[2]))
					except: TPR= 0
                                        try: Sensitivity = float(l[0])/(float(l[0])+float(l[3]))
					except: Sensitivity = 0
                                        Taxa = l[4]
                                        with open(output.ROC, "a") as O:
                                                O.write("\t".join([str(TPR), str(Sensitivity), str(Thresh), Taxa])+"\n")
