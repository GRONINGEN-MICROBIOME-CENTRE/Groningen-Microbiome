...
parser.add_argument('--N_strains', '-om', action = 'store', default = '', dest = 'N_strains', required=True)
...
Number_strains = int(parser.parse_args().N_strains)
if "%" in N_mutations:
	Proportion = True
	...
	with open(FASTA, "r") as File:
		for record in SeqIO.parse(File, "fasta"):
			...
			for Position in Position_mutated:
				...
				dic_mutations[NAME][Position] = [Prev, New, chrom, FASTA+"_"+str(n)]
				SEQ[Position] = New
...
with open(output_m,"w") as O:
	for Genome in dic_mutations:
		for mutation in dic_mutations[Genome]:
			Line = "\t".join([Genome, str(mutation+1),dic_mutations[Genome][mutation][2], dic_mutations[Genome][mutation][0], dic_mutations[Genome][mutation][1]], dic_mutations[Genome][mutation][-1]) + "\n"
			O.write(Line)			
