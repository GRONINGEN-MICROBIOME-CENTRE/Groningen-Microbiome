...
# 21 July 2016 
# Ettje Tigchelaar
# Update 25 march 2020 by Trishla Sinha: translating the script to fit our english questionaire 
...
# name of dataframe: RO (new file Bas imported DAG3_ROME3_NEW_MARCH_2020_TS.dat )
# Variable names questions: ROME1 to ROME26 
# Add a column sex to this data R0$R_2geslacht and label gender man vrouw 
...
for(i in c("ROME3")){
  RO$j <- ifelse(RO[,i]=="no",0,
                 ifelse(grepl("yes",RO[,i]),1,NA))
  colnames(RO)[colnames(RO)=='j'] <- c(paste(colnames(RO[i]),"s",sep=""))
}

for(i in c("ROME15","ROME17","ROME18","ROME20","ROME22","ROME24","ROME26")){
  RO$j <- ifelse(RO[,i]=="no",0,
...
## diagnostic criteria IBS
  ...
  c = ifelse(RO$ROME2s==0 | RO$ROME2s==2 |(is.na(RO$ROME2s) & RO$GESLACHT==1),1,0) # pain not associated to menstrual bleeding
