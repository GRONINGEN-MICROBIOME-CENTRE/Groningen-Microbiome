####below are main codes for microbial co-abundance network analysis (for questions, please contact Lianmin Chen, lianminchen@yeah.net)
...
for(i in 1:nrow(data)){
  data$fdr[i]=(length(which(abs(data[,3:102])>=abs(data$coeffifient)[i]))/100)/i #3:102 columns represent fake coefficient
}


###3.heterogeneity test (Cochran-Q test) on edges (R language)
library(meta)
TE=c(cor_lld,cor_500fg,cor_300ob,cor_ibd) #coefficient/odds ratio of certain edge in four cohorts
TEse=c((1-cor_lld^2)/sqrt(1135-2),(1-cor_500fg^2)/sqrt(450-2),(1-cor_300ob^2)/sqrt(298-2),(1-cor_ibd^2)/sqrt(496-2))  #calculate se of coefficient
