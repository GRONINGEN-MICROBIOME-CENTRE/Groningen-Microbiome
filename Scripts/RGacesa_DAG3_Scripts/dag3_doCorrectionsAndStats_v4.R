...
# print (paste0(' >> ',sum(res2$PCorr < 0.05),' / ',nrow(res),' Features are *non-normal* !' ))
...
dfNormLcTech <- linearCorrectMGPwy(dfNorm,corrNames=c("vol.ul","postclean.reads","conc.ng.ul"),negIsZero=F,debug = F)
...
dfNormLcASB <- linearCorrectMGPwy(dfNormLcTech,corrNames=c("age","gender","BMI"),negIsZero=F,debug = T)
write.table(dfNormLcASB,'data_processed/profiling/DAG3_taxa_pwys_CARD_VFDB_filt_rsc_asnor_corrtechASB.csv',sep=',',row.names = F)

# >> multi-correct for comparison
dfNormLcMulti <- linearCorrectMGPwy(dfNorm,corrNames=c("vol.ul","postclean.reads","conc.ng.ul","age","gender","BMI"),negIsZero=F,debug = F)
write.table(dfNormLcMulti,'data_processed/profiling/DAG3_taxa_pwys_CARD_VFDB_filt_rsc_asnor_corrtechASB_multi.csv',sep=',',row.names = F)
