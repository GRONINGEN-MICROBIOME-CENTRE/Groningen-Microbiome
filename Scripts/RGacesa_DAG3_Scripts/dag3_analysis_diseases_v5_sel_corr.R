# ===========================================
# DAG3 ANALYSIS, diseases [selected ones]
# >> version 5 (control group is whole population)
# ===========================================
...
# load data (filtered - asin(sqrt) normalised - rescaled - corrected for technicals, age, sex, BMI)
...
plotTopX <- 5 # if > 0, plots top X regardless of stats
minAge <- 12 # throw out kids!
diagnosticPlots <- T

# define controls
# ==============
specificControls = F
controlIDs <- phenos$dag3_sampleid[phenos$none.of.the.disorders=="Y"]
sexAgeBMIMatch <- F
maxCtrlRatio <- 100
# =============
# ==========================================
...
# load phenotypes
phenos <- read.table('../DAG3_meta/v2/health/curated/DAG3_T1T2_Health_small.csv',sep=',',header=T,quote = '"')
colnames(phenos) <- tolower(colnames(phenos))
#  -- select phenotypes to test --
#selCols <- colnames(phenos)[c(3:46,54:66,68:length(colnames(phenos)))]
selCols <- colnames(phenos)[c(2:length(colnames(phenos)))]
phenNames  <- selCols
...
for (phen in selCols) {
  ...
  if (sexAgeBMIMatch) {
    ...
    drk <- phDFm2[,c("ID",phen,"gender","age","gender","BMI")]
...
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("diversities"),nrToPlot = 10,outFile = paste0(hmFolder,"/heatmap_diversities_stats.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,clusterPhenos = cPH,subsetPhenos = ssp,metric="statSig")
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("diversities"),nrToPlot = 10,outFile = paste0(hmFolder,"/heatmap_diversities_effects.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,clusterPhenos = cPH,subsetPhenos = ssp,metric="meanEffectRatio")
...
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("taxPrevalences"),nrToPlot = 30,outFile = paste0(hmFolder,"/heatmap_taxa_g_prevalence_stats.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,subsetFeatures="g__",clusterPhenos = cPH,subsetPhenos = ssp,
                          metric="statSig")
...
# SPECIES
#  > prevalences
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("taxPrevalences"),nrToPlot = 50,outFile = paste0(hmFolder,"/heatmap_taxa_s_prevalence_stats.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,subsetFeatures="s__",clusterPhenos = cPH,subsetPhenos = ssp,
                          metric="statSig")
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("taxPrevalences"),nrToPlot = 50,outFile = paste0(hmFolder,"/heatmap_taxa_s_prevalence_effects.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,subsetFeatures="s__",clusterPhenos = cPH,subsetPhenos = ssp,
                          metric="meanEffectRatio")
#  > abundances
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("taxAbundances"),nrToPlot = 50,outFile = paste0(hmFolder,"/heatmap_taxa_s_abundances_stats.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,subsetFeatures="s__",clusterPhenos = cPH,subsetPhenos = ssp,
                          metric="statSig")
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("taxAbundances"),nrToPlot = 50,outFile = paste0(hmFolder,"/heatmap_taxa_s_abundances_effects.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,subsetFeatures="s__",clusterPhenos = cPH,subsetPhenos = ssp,
                          metric="meanEffectRatio")

# PATHWAYS
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("pwyAbundance"),nrToPlot = 40,outFile = paste0(hmFolder,"/heatmap_pwys_stats.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,clusterPhenos = cPH,subsetPhenos = ssp,addXextra=600)
p <- aggregatePlotHeatmap(tblFolder,doFeatures=c("pwyAbundance"),nrToPlot = 40,outFile = paste0(hmFolder,"/heatmap_pwys_effects.png"),
                          multiTest=mT,pVcutoff = FDRc,enrichmentCutoff = enrC,clusterPhenos = cPH,subsetPhenos = ssp,addXextra=600,
                          metric="meanEffectRatio")
